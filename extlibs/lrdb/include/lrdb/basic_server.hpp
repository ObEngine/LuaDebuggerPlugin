...
#define LRDB_SERVER_PROTOCOL_VERSION "2"
    ...
    public:
        ...
        StreamType& command_stream()
        {
            return command_stream_;
        };

    private:
        void init()
        {
            debugger_.set_pause_handler(
                [&](debugger&)
                {
                    send_pause_status();
            ...
            command_stream_.on_data = [=](const std::string& data) { execute_message(data); };
        ...
        bool add_breakpoint_request(response_message& response, const json::value& param)
        {
            ...
            if (has_source && has_line)
            {
                ...
                int line = static_cast<int>(param.get<json::object>().at("line").get<double>());
            ...
            {
                response.error = response_error(response_error::InvalidParams, "invalid params");
            }
            return send_response(response);
        }

        bool clear_breakpoints_request(response_message& response, const json::value& param)
        {
            bool has_source = param.get("file").is<std::string>();
            bool has_line = param.get("line").is<double>();
            if (!has_source)
        ...
        {
            const debugger::line_breakpoint_type& breakpoints = debugger_.line_breakpoints();
        ...
        bool get_local_variable_request(response_message& response, const json::value& param)
        {
            ...
            if (has_stackno)
            {
                ...
                if (int(callstack.size()) > stack_no)
                {
                    ...
                    std::cout << "LOCALS AS STR " << response.result.serialize() << std::endl;
                    return send_response(response);
                }
            }
            response.error = response_error(response_error::InvalidParams, "invalid params");
        ...
        bool get_upvalues_request(response_message& response, const json::value& param)
        {
            if (!param.is<json::object>())
            {
                response.error = response_error(response_error::InvalidParams, "invalid params");
            ...
            {
                int stack_no
                    = static_cast<int>(param.get<json::object>().at("stack_no").get<double>());
                auto callstack = debugger_.get_call_stack();
                if (int(callstack.size()) > stack_no)
                {
        ...
        bool eval_request(response_message& response, const json::value& param)
        {
            ...
            bool use_global = !param.get("global").is<bool>() || param.get("global").get<bool>();
            bool use_upvalue = !param.get("upvalue").is<bool>() || param.get("upvalue").get<bool>();
            bool use_local = !param.get("local").is<bool>() || param.get("local").get<bool>();
        ...
        bool get_global_request(response_message& response, const json::value& param)
        {
            int depth = param.get("depth").is<double>()
                ? static_cast<int>(param.get("depth").get<double>())
                : 1;
            response.result = debugger_.get_global_table(depth + 1); //+ 1 is global table self
        ...
        {
            typedef bool (basic_server::*exec_cmd_fn)(
                response_message & response, const json::value& param);
...
#undef LRDB_DEBUG_COMMAND_TABLE
            ...
            else
            {
                response.error = response_error(
                    response_error::MethodNotFound, "method not found : " + req.method);
