...
#pragma warning(pop)
        ...
        inline void push_json(lua_State* L, const json::value& v)
        {
            ...
            else if (v.is<json::object>())
            {
                ...
                for (json::object::const_iterator itr = obj.begin(); itr != obj.end(); ++itr)
                {
  ...
  std::vector<bool> valid_lines_on_function() {
    ...
    return ret;
        ...
        std::vector<json::value> eval(const char* script, bool global = true, bool upvalue = true,
        ...
        std::vector<json::value> eval(const char* script, std::string& error, bool global = true,
            bool upvalue = true, bool local = true, int object_depth = 1)
        {
            int stack_start = lua_gettop(state_);
            int loadstat = luaL_loadstring(state_, (std::string("return ") + script).c_str());
            if (loadstat != 0)
            {
                lua_pop(state_, 1);
    ...
    private:
        void create_eval_env(bool global = true, bool upvalue = true, bool local = true)
        {
    ...
    public:
        ...
        bool is_available()
        {
            return valid_ && debug_info::is_available();
        }
        ~stack_info()
        {
            debug_ = 0;
    ...
    public:
        ...
        void add_breakpoint(const std::string& file, int line, const std::string& condition = "",
        ...
        void clear_breakpoints(const std::string& file, int line = -1)
        {
            line_breakpoints_.erase(
                std::remove_if(line_breakpoints_.begin(), line_breakpoints_.end(),
                    ...
                    { return (line < 0 || b.line == line) && (b.file == file); }),
                line_breakpoints_.end());
        }
        /// @brief clear breakpoints
        void clear_breakpoints()
        {
            line_breakpoints_.clear();
        }
        ...
        /// return.
        void set_tick_handler(tick_handler_type handler)
        {
        ...
        breakpoint_info* current_breakpoint()
        {
    ...
    private:
        void sethook()
        {
            ...
            lua_sethook(state_, &hook_function, LUA_MASKCALL | LUA_MASKRET | LUA_MASKLINE, 0);
        }
        void unsethook()
        {
            if (state_)
        ...
        bool breakpoint_cond(const breakpoint_info& breakpoint, debug_info& debuginfo)
        {
        ...
        bool breakpoint_hit_cond(const breakpoint_info& breakpoint, debug_info& debuginfo)
        {
            if (!breakpoint.hit_condition.empty())
            {
                json::array condret = debuginfo.eval(
                    (std::to_string(breakpoint.hit_count) + breakpoint.hit_condition).c_str());
        ...
        void hookline()
        {
            ...
            if (current_breakpoint_ && breakpoint_cond(*current_breakpoint_, current_debug_info_))
            {
