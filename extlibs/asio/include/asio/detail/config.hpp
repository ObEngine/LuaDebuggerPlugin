...
#pragma message("Please define _WIN32_WINNT or _WIN32_WINDOWS appropriately. For example:\n"       \
...
// for threads support.
    ...
    || (defined(__MACH__) && defined(__APPLE__)) || defined(__FreeBSD__) || defined(__NetBSD__)    \
...
#if defined(ASIO_HAS_IOCP)                                                                         \
    || !defined(ASIO_WINDOWS) && !defined(ASIO_WINDOWS_RUNTIME) && !defined(__CYGWIN__)
...
#define ASIO_STATIC_CONSTANT(type, assignment) BOOST_STATIC_CONSTANT(type, assignment)
